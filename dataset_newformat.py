...
class ArucoDataset(Dataset):
	...
	def __getitem__(self, idx):
		...
		num_markers = random.randint(4, 8)  # Variable number of markers
		# num_markers = 12
		labels = []
		labels_old = []
		...
		for marker_id in selected_ids:
			...
			while True:
				...
				if self.is_valid(x,y,marker,w,h,outline_corner,labels_old):
					break
			...
			label_old = corners.flatten().tolist()
			...
			label = [center_x / w, center_y / h, width / w, height / h]
			labels.append(label)
		...
		if self.transform:
			...
			self.scale_y = 1
		grid_size = 14
		grid_labels = torch.zeros((grid_size, grid_size, 5))
		for label in labels:
			grid_x = int(label[0] * grid_size)
			grid_y = int(label[1] * grid_size)
			grid_x = max(0, min(grid_x, grid_size - 1))
			grid_y = max(0, min(grid_y, grid_size - 1))

			cell_x = label[0] * grid_size - grid_x
			cell_y = label[1] * grid_size - grid_y
			cell_w = label[2] 
			cell_h = label[3]
			grid_labels[grid_y, grid_x, :] = torch.tensor([1.0, cell_x, cell_y, cell_w, cell_h])
		return image.cuda(), grid_labels.cuda()
	...
	def save_and_visualize(self, image, grid_labels, save_path, vis=False):
		...
		image = image * 255  # Ensure the image is scaled properly
		...
		for grid_y in range(grid_size):
			for grid_x in range(grid_size):
				if grid_labels[grid_y, grid_x, 0] > 0.7:  # Check if tag is present
					cell_x, cell_y, cell_w, cell_h = grid_labels[grid_y, grid_x, 1:]
					cx = (grid_x + cell_x) * (img_w / grid_size)/self.scale_x
					cy = (grid_y + cell_y) * (img_h / grid_size)/self.scale_y
					width = cell_w * img_w/self.scale_x
					height = cell_h * img_h/self.scale_y
...
if __name__ == '__main__':
	...
	for batch_idx, (images, labels) in enumerate(dataloader):
		...
		for idx in range(batch_size):
			...
			dataset.save_and_visualize(image, batch_labels, f'batch{batch_idx}_image{idx}_visualization.png',vis=True)